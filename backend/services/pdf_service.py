...
def create_scores_chart(scores: dict) -> io.BytesIO:
    ...
    bars = ax.barh(domains, values, color=bar_colors, height=0.6, zorder=3)
    ...
    plt.savefig(img_buffer, format='png', dpi=150, facecolor=fig.get_facecolor())
    plt.close()
    img_buffer.seek(0)
    return img_buffer
